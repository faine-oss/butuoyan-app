...
- 手机直接下载：`https://faine-oss.github.io/butuoyan-app/downloads/butuoyan-v0.6.0-android.apk`
- GitHub 备用下载：在右侧 Releases 中下载 `butuoyan-v0.6.0-android.apk`
...
早期 V0.2 本地测试包使用 Android 调试证书，与 V0.3 之后的正式证书不同，不能直接覆盖安装。首次切换到正式版前请确认测试版中没有需要保留的数据，然后卸载测试版再安装正式版。V0.3 至 V0.6.0 的正式版使用同一签名，可覆盖更新；安装前请导出完整备份，不要卸载现有正式版。

V0.6.0 补齐跨页面专注计时、任务完成确认、精确到时刻的 DDL 重排、高清多页计划书导出、任务筛选与回收站，以及保存失败保护和截止前提醒。详细改动见 [发布说明](releases/v0.6.0.md)。系统通知可能受锁屏、省电策略影响；实际打印与不同手机的提醒行为仍需验证。
...
```powershell
Get-FileHash -Algorithm SHA256 -LiteralPath ".\butuoyan-v0.6.0-android.apk"
```
